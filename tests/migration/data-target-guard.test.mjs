...
test('VM107 bootstrap requires exact confirmation immediately before delete and restore', () => {
  ...
  assert.match(
    bootstrap,
    /for service in "\$\{build_services\[@\]\}"; do\s+if \[\[ "\$service" == api && "\$candidate_api_built" == true \]\]; then\s+continue\s+fi\s+docker compose --env-file "\$SECRET_ENV_PATH" build "\$service"\s+done/,
  );
  ...
  assert.match(
    bootstrap,
    /prepare_runtime_env\s+verify_public_email_readiness\s+reconcile_disposable_database_credentials\s+start_stack/,
  );
