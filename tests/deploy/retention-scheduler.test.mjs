...
const expectedExecStart = '/usr/bin/env RETENTION_PURGE_STAGE=application_data RETENTION_PURGE_DRY_RUN=false RETENTION_PURGE_EXECUTE_CONFIRM=purge-expired-application-data /usr/bin/node /opt/lunchlineup/scripts/invoke-retained-record-purge.mjs';
...
test('retention scheduler is pinned to confirmed 30-day application-data execution', () => {
  ...
  assert.doesNotMatch(`${service}\n${envExample}`, /RETENTION_PURGE_EXECUTE_CONFIRM=purge-expired-retained-records/);
...
test('retained-record scheduler install docs are end-to-end and verify-first', () => {
  const systemdReadme = read('infrastructure/systemd/README.md');
  const runbook = read('docs/runbooks/data-retention-delete-export.md');
  const expectedExecPattern = new RegExp(expectedExecStart.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  ...
  assert.match(systemdReadme, /install -m 0644 infrastructure\/systemd\/lunchlineup-retention-purge\.timer \/etc\/systemd\/system\/lunchlineup-retention-purge\.timer/);
  assert.match(systemdReadme, /systemd-analyze verify \/etc\/systemd\/system\/lunchlineup-retention-purge\.service \/etc\/systemd\/system\/lunchlineup-retention-purge\.timer/);
  assert.match(systemdReadme, /systemctl daemon-reload/);
  assert.match(systemdReadme, /systemctl enable --now lunchlineup-retention-purge\.timer/);
  assert.match(systemdReadme, /systemctl start lunchlineup-retention-purge\.service/);
  assert.match(systemdReadme, /journalctl -u lunchlineup-retention-purge\.service -n 50 --no-pager/);
  ...
  assert.match(runbook, /systemctl enable --now lunchlineup-retention-purge\.timer/);
  assert.match(runbook, /\/var\/lib\/lunchlineup\/proofs\/retention-purge-latest\.json/);
  assert.match(runbook, /\/var\/lib\/node_exporter\/textfile_collector\/lunchlineup_retention_purge\.prom/);
});

test('retained-record scheduler uses token, proof, lock, and metrics files', () => {
  const envExample = read('infrastructure/systemd/lunchlineup-retention-purge.env.example');
  const runbook = read('docs/runbooks/data-retention-delete-export.md');
  const systemdReadme = read('infrastructure/systemd/README.md');
...
test('retained-record scheduler docs name monitoring and alert ownership', () => {
  const runbook = read('docs/runbooks/data-retention-delete-export.md');
  const systemdReadme = read('infrastructure/systemd/README.md');

  for (const content of [runbook, systemdReadme]) {
    assert.match(content, /Platform on-call owns `RetentionPurgeTelemetryMissing`, `RetentionPurgeStale`, `RetentionPurgeFailed`, and `RetentionPurgeCandidatesReady`/);
