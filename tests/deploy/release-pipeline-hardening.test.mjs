...
test('registry-writing image job is unreachable from pull requests', () => {
  ...
  assert.equal(imageBuilds.length, 17);
...
test('internal beta local pipeline keeps isolated source, active scanners, exact archives, and signed VM107 launch', () => {
  ...
  assert.doesNotMatch(imageScan, /--exit-code 0/);
  assert.match(imageScan, /Release archive escapes artifact root/);
  const imageReportVerifier = read('scripts/verify-internal-ci-image-reports.mjs');
  assert.match(imageReportVerifier, /report\.SchemaVersion!==2/);
  assert.match(imageReportVerifier, /report\.Results\.length===0/);
  assert.match(read('.dockerignore'), /\*\*\/\.pytest_cache\//);
