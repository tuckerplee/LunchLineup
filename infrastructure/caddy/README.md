...
The proxy forwards `/health`, `/api/health`, and `/api/v1/*` to the retained NestJS service. `/api/v2/*` is routed independently to the contract-first Fastify service before the v1 matcher, and all other paths go to Next.js. Compose healthchecks use the unexposed `http://127.0.0.1:2015/health` listener, so container health is independent of public host routing. Compose passes the explicit immutable deployment SHA as `DEPLOY_RELEASE_SHA`, and Caddy returns it in `X-LunchLineup-Release`. The canonical public root `/` is the deploy-time web probe: its release header must match the source SHA, and its body must contain the rendered LunchLineup heading plus a `/_next/static/` asset reference. This prevents an API response, generic proxy page, or stale prior-release page from satisfying the gate. The proxy also applies a 10 MB request body cap plus baseline browser security headers, including HSTS for public HTTPS deployments. API responses receive `Cache-Control: no-store` at the edge to avoid caching tenant data or metrics.
