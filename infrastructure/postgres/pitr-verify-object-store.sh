...
pitr_upload_encrypted "${PROBE_FILE}" "${PROBE_REMOTE}" >/dev/null
pitr_mc stat "${PROBE_REMOTE}" >/dev/null
...
if pitr_mc rm --versions --force "${PROBE_REMOTE}" >/dev/null 2>&1; then
...
OBJECT_RETENTION="$(pitr_mc retention info "${PROBE_REMOTE}")" \
...
printf 'pitr_object_store_ready writer=%s versioning=enabled object_lock=compliance retention_days=%s delete=denied remote=%s\n' \
  "${WRITER_NAME}" "${PITR_OBJECT_LOCK_RETENTION_DAYS}" "${PROBE_REMOTE}"
