...
API v2 exposes `publicId` UUIDs for users, locations, schedules, shifts, solve jobs, time cards, and persisted time-card breaks; internal primary keys remain storage-only. Database defaults keep older v1 writers compatible during the migration window. `ScheduleChangeSet` is the tenant-RLS, idempotent ledger for one atomic scheduling aggregate mutation and one schedule revision increment.
