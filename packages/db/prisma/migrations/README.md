...
- `20260716_zzzzzz_billing_exact_once.sql`: adds authoritative Stripe paid-through state, subscription-binding and paid-entitlement indexing, legacy-readable immutable `CreditTransaction.balanceAfter`, a `NOT VALID` required-balance check that rejects every new null insert without validating historical null rows, nonnegative settlement validation, and a database trigger preventing settlement-result rewrites.
