...
- `pre_20260717_legacy_break_window_reanchor.sql`: re-anchors only the exact legacy three-break 10/30/10 paid/unpaid/paid pattern to its parent shift while preserving durations and relative spacing, and rejects every unrecognized invalid or overlapping shape.
- `pre_20260717_prisma_composite_parent_keys.sql`: stages and catalog-verifies Prisma-owned composite identities on user, role, location, schedule, and shift parents before tenant-consistency foreign keys are synchronized.
