...
- `test_solver.py`: tests for typed default and custom-policy breaks, break-relief working coverage, solver feasibility, bounds, existing-plus-new weekly limits, cross-location existing-shift overlap exclusion, Monday calendar-week boundaries, explicit empty availability as unavailable, touching/overlapping availability-window union coverage with internal-gap rejection, legacy direct-call omission behavior, DST/location timezone output, daily fallback demand, exact skill-qualified demand-window output, break containment across protected skill boundaries, two-worker general/cashier overlap reuse, and independent overlapping skill coverage.
