...
The exact 121-operation API-01 catalog includes the native session operation, six native location operations, 16 native People operations, nine native Operations resources, six native Time Card operations, and four native workspace Settings operations; 79 operations remain behind compatibility owners. The People module exposes public UUID staff/role resources, enforces live access revalidation for mutations, and produces encrypted staff-invitation commands while the worker remains the sole sender. The Operations module exposes public schedule/shift/roster read models and direct lunch/break persistence with tenant-RLS, credit settlement, idempotency, and revision fencing. The Time module exposes public time-card and break resources, preserves exact durable clock-in replay and credit settlement across the v1 cutover, and keeps clock-out recovery available after entitlement loss. The Settings module owns tenant-scoped workspace settings, strict request contracts, OIDC-only safety gating, and redacted security-policy audits. The retained operations preserve mature v1 behavior through `platform/retained-application.bridge.ts`, with fixed upstream authority, bounded request/response sizes, no redirects except the two declared OIDC operations, sanitized RFC 9457 errors, and no wildcard route. Its identifier translators rewrite only declared exact fields and retain no generic-ID fallback. API-02 owns deleting those compatibility operations as typed native domain modules land; the catalog may not grow.
