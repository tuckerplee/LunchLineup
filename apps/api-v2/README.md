...
The exact 121-operation API-01 catalog includes the native session operation, six native location operations, and 16 native People operations; 98 operations remain behind compatibility owners. The People module exposes public UUID staff/role resources, enforces live access revalidation for mutations, and produces encrypted staff-invitation commands while the worker remains the sole sender. The retained operations preserve mature v1 behavior through `platform/retained-application.bridge.ts`, with fixed upstream authority, bounded request/response sizes, no redirects except the two declared OIDC operations, sanitized RFC 9457 errors, and no wildcard route. Its identifier translators rewrite only declared exact fields and retain no generic-ID fallback. API-02 owns deleting those compatibility operations as typed native domain modules land; the catalog may not grow.
