...
One change set can create, update, assign, move, or delete up to 100 shifts. It evaluates the final state, so a multi-shift swap is valid when its result has no overlap even if sequential row updates would temporarily overlap.
