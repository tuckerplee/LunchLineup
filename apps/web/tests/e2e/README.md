...
- `internal-beta-interaction-desktop.proof.ts`: launch-only release-image proof for no-op/cancel/outside-drop pointer safety, exact proposed and committed targets, object-local failed-move recovery, keyboard fallback, overnight Calendar/Lunch identity, supported Lunch actions/cost disclosure, and explicit Time Cards selections.
- `internal-beta-interaction-support.ts`: exact disposable-seed, shift-readback, pointer-geometry, and change-set capture helpers shared by the launch interaction proof.
- `internal-beta-interaction-touch.proof.ts`: real Chromium touch-emulation proof that schedule scrolling cannot move a shift and only the dedicated handle activates the Move fallback.
- `lunch-break-recovery.spec.ts`: focused mock-backed Chromium proof for exact schedule-backed 23:00-07:00 preservation, pre-confirmation mutation exclusion, explicit one-credit setup copy, uncapped 30-person search/selection, generation A-to-B-to-A plus response loss/reload with one A key/debit, and shared-origin two-page identical setup/generation races with one deterministic key/debit each.
- `mock-api.mjs`: local Playwright API with native v2 browser-session validation/refresh, public browser/admin user IDs and opaque session scopes, bounded location lists, exact location reads, count summaries, payroll permissions, recurring and dated staff-availability profile fixtures, metered actions requiring an active paid subscription plus separate credits, and idempotent shift updates and lunch/break generation.
- `month-volume-workflows.spec.ts`: opt-in full-stack API-v2 ten-person schedule, native staff invitations, keyed change-set/lunch generation volume, credit settlement, replay, and persisted readback contract test.
...
- `staff-admin-safety.spec.ts`: manager scheduling-profile workflow with public location IDs, authoritative PDF-import cost preflight and settlement, fail-closed read recovery, skills, recurring overnight availability, dated availability exceptions, Staff-default invitation, destructive-action confirmation, public-ID-bound MFA recovery, capability separation, and responsive platform-admin coverage.
- `staff-onboarding.spec.ts`: focused Chromium coverage for explicit email/PIN onboarding, delivery-disabled fail-closed UI, cross-mode payload exclusion, generated PIN copy/acknowledgement, and staged role Save/Cancel behavior.
- `stress-workflows.spec.ts`: mock-backed lunch guided-screen H1/Axe, Strict Mode scoped-read count, 503 stale-row invalidation, delayed setup/scheduled/manual A cleanup versus newer B busy ownership through exactly-once B success, active-location rows/banners/previews/planner/guide fencing, public-ID and opaque-scope recovery storage, committed-response-loss A-to-B-to-A exact-once setup recovery, and idempotent setup/save coverage plus opt-in full-stack A/B stress flows.
- `time-card-target-safety.spec.ts`: focused mock-backed Chromium proof that My Time remains session-bound, Team Time cannot write before explicit person/location selection, named controls and confirmations identify the target, and person/location changes clear sensitive drafts and correction state.
...
- `verify-internal-beta-interaction-proof.mjs`: fail-closed JSON-report verifier requiring every desktop/touch critical case exactly once with no retry, skip, fixme, failure, or missing result and emitting the exact-candidate receipt.
