...
describe('staff workspace permission contract', () => {
  it('keeps invitation and destructive administration on distinct permissions', () => {
    ...
    expect(workspaceSource).toContain('{canAdminister && user.id !== currentUserId && !user.email ? (');
    expect(workspaceSource).toContain('{canAdminister && user.id !== currentUserId ? (');
    ...
    expect(workspaceSource).toContain('{canAssignRoles && canReadRoles && user.id !== currentUserId ? (');
