...
- `playwright-config-contract.test.ts`: source contract keeping default mock Playwright on a serialized Next development server, requiring native v2 browser-session/public-ID fixtures and an explicit dependency-health probe, validating the closed-beta render override, and preserving external/full-stack targeting.
- `staff-permission-contract.test.ts`: static server-to-client capability coverage keeping manager scheduling-profile/invitation access separate from admin-only PIN reset and staff removal controls, while proving the typed profile client and dated availability/time-off editor stay on the protected atomic endpoint.
...
- `scheduler-gesture.test.ts`: eight-pixel activation, delayed touch intent, quarter-hour quantization, board-scoped semantic target IDs, and fail-closed drop-decision coverage.
- `scheduler-responsive-accessibility.test.ts`: fixed-scale mobile timeline, desktop-fit preservation, separate card/handle semantics, modifier copy, board-local cancellation wiring, keyboard Move dialog, published lock, accessible break markers, horizontal keyboard inspection, location-timezone initial-date alignment, date naming, staff-row semantics, and compact-label contrast coverage.
