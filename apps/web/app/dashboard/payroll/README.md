...
- `PayrollWorkspace.tsx` - Manager-task framing for period setup, review, approval, lock, export, and provider confirmation.
- `PayrollAuditDetails.tsx` - Closed-by-default disclosure for revision, idempotency, SHA-256, and immutable row evidence.
...
- `PayrollPeriodDetail.tsx` - Exact-permission time-card review, approval, payroll lock, paid export, and amendment composition in manager language.
...
- `PayrollReconciliation.tsx` - Provider-result confirmation with signed totals, rejected/all-accepted correction, bounded line pages, partial states, and exact-request replay.
...
- `payroll.module.css` - Compact responsive table, form, status, focus, confirmation, and closed audit-disclosure styles down to 375px.
...
A locked or exported batch is not payroll-final. External payroll remains authoritative until every deterministic export line is accepted and signed provider total minutes exactly match the batch. The primary screen stays task-oriented; revision, hash, idempotency, and immutable evidence remain available under the collapsed Audit details disclosure. Export lines load in explicit bounded pages; reconciliation records up to 500 explicitly loaded outcomes per request, permits rejected-to-accepted correction, and keeps accepted lines editable while an all-accepted provider total is still wrong. Ambiguous transport retains the exact payload only in memory for same-view replay. Payroll session metadata is cleared on route exit, sign-out, and account change.
