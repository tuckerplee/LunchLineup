...
The workspace never drains location pages automatically. Refresh replaces the visible first page; Load more appends and de-duplicates the next opaque-cursor page, preserving first-location creation and recovery behavior. Every location `id` here is the native v2 public UUID, including edit/deactivation and scheduling navigation; storage IDs never enter browser state.
