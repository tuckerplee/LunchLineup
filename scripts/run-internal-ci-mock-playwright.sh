...
E2E_FULL_STACK=0 E2E_MOCK_API=1 E2E_SIGNUP_MODE=closed_beta PLAYWRIGHT_JSON_OUTPUT_NAME="$output/results.json" npx playwright test --reporter=json >"$output/test.log" 2>&1
...
const fs=require('node:fs');const [reportPath,output,sourceSha]=process.argv.slice(2),report=JSON.parse(fs.readFileSync(reportPath));const stats=report.stats??{};for(const key of ['expected','skipped','unexpected','flaky'])if(!Number.isSafeInteger(stats[key])||stats[key]<0)throw new Error('Invalid Playwright JSON statistics.');if(stats.unexpected!==0||stats.skipped!==0||stats.flaky!==0)throw new Error('Mock Playwright is not complete, deterministic, and clean.');fs.writeFileSync(output,JSON.stringify({sourceSha,browsers:['chromium','firefox'],fullStack:false,mockApi:true,passed:stats.expected,failed:stats.unexpected,skipped:stats.skipped,flaky:stats.flaky},null,2)+'\n',{flag:'wx',mode:0o600});
NODE
