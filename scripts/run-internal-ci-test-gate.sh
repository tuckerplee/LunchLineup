...
if [[ "$suite" == engine ]]; then
  node -e 'const fs=require("node:fs"),coverage=JSON.parse(fs.readFileSync(process.argv[1],"utf8")),percent=coverage.totals?.percent_covered;if(typeof percent!=="number"||!Number.isFinite(percent)||percent<90)throw new Error(`Engine coverage ${percent} is below 90.00%.`);fs.writeFileSync(process.argv[2],JSON.stringify({sourceSha:process.argv[3],suite:"engine",coveragePercent:percent,coverageThreshold:90},null,2)+"\n",{flag:"wx",mode:0o600});' "$output/coverage.json" "$artifact_root/details/$gate.json" "$CI_COMMIT_SHA"
else
...
[[ -f "$output/coverage.json" ]] && evidence+=(--evidence "$output/coverage.json")
