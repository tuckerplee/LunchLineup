...
cleanup_failed_stage(){ local status=$?; if [[ "$status" -ne 0 && -f "$env_file" ]]; then "${compose[@]}" --profile ops down -v --remove-orphans >/dev/null 2>&1 || true; fi; exit "$status"; }
