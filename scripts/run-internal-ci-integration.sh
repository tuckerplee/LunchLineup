...
cleanup(){ if [[ -n "$graph_root" && -n "$container_run_root" ]]; then container rm -f "$postgres" "$redis" "$rabbitmq" >/dev/null 2>&1 || true; fi; if [[ -n "$graph_root" && -e "$graph_root" && ! -L "$graph_root" ]]; then case "$graph_root" in "$RUNNER_TEMP"/*) rm -rf -- "$graph_root";; esac; fi; if [[ -n "$runtime_root" && -e "$runtime_root" && ! -L "$runtime_root" ]]; then case "$runtime_root" in /tmp/llr.*) rm -rf -- "$runtime_root";; esac; fi; }; trap cleanup EXIT
umask 022
...
container rm -f "$postgres" "$redis" "$rabbitmq" >/dev/null; for resource in "$postgres" "$redis" "$rabbitmq"; do if container inspect "$resource" >/dev/null 2>&1; then exit 1; fi; done; rm -rf -- "$venv"; test "$graph_root" = "$(realpath -e "$graph_root")"; case "$graph_root" in "$RUNNER_TEMP"/*) ;; *) exit 1;; esac; rm -rf -- "$graph_root"; test ! -e "$graph_root"; graph_root=''; test "$runtime_root" = "$(realpath -e "$runtime_root")"; case "$runtime_root" in /tmp/llr.*) ;; *) exit 1;; esac; rm -rf -- "$runtime_root"; test ! -e "$runtime_root"; runtime_root=''; container_run_root=''; trap - EXIT; printf 'cleanup=passed\n' >"$output/cleanup.log"
