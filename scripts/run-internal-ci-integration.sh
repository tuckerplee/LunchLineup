...
docker rm -f "$postgres" "$redis" "$rabbitmq" >/dev/null 2>&1 || true
pg_password="pg_$(openssl rand -hex 24)"; app_password="app_$(openssl rand -hex 24)"; mq_password="mq_$(openssl rand -hex 24)"
docker run -d --name "$postgres" --network slirp4netns:port_handler=slirp4netns -p 127.0.0.1::5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD="$pg_password" -e POSTGRES_DB=lunchlineup_test postgres:16-alpine@sha256:cf78e76683b9ca8c5733cbbdce6c9262b45b6767934dd0a95e671f9a0fc20685 >/dev/null
docker run -d --name "$redis" --network slirp4netns:port_handler=slirp4netns -p 127.0.0.1::6379 redis:7-alpine@sha256:e7723ff73d963f5cc6d9c4643ea3d989527a402a319239054e9472a7fb9219a2 >/dev/null
docker run -d --name "$rabbitmq" --network slirp4netns:port_handler=slirp4netns -p 127.0.0.1::5672 -e RABBITMQ_DEFAULT_USER=lunchlineup_ci -e RABBITMQ_DEFAULT_PASS="$mq_password" rabbitmq:4-alpine@sha256:ae585b93b24b77f7281320c7d1e62b3098acba91eb14b1e53a9716584c95c7e9 >/dev/null
...
docker rm -f "$postgres" "$redis" "$rabbitmq" >/dev/null; for resource in "$postgres" "$redis" "$rabbitmq"; do if docker inspect "$resource" >/dev/null 2>&1; then exit 1; fi; done; rm -rf -- "$venv"; test "$runtime_root" = "$(realpath -e "$runtime_root")"; case "$runtime_root" in /tmp/llr.*) ;; *) exit 1;; esac; rm -rf -- "$runtime_root"; test ! -e "$runtime_root"; runtime_root=''; trap - EXIT; printf 'cleanup=passed\n' >"$output/cleanup.log"
