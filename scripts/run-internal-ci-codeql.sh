...
"$CODEQL_CLI" database analyze "$db_root/$language" "$suite" --ram="$codeql_ram_mb" --format=sarifv2.1.0 --output="$output/codeql-$language.sarif" --sarif-category="/language:$language" --download=false >"$output/$language-analyze.log" 2>&1
