...
- Both target `RETENTION_PURGE_URL=https://lunchlineup.com/api/v2/admin/retention/purge-expired` and read `RETENTION_PURGE_TOKEN_FILE=/run/secrets/retention_purge_token`. API v2 requires the explicit bearer and forwards it only to the fixed private service-token gate; the API container mounts the same generated secret through `RETENTION_PURGE_SERVICE_TOKEN_FILE` and `RETENTION_PURGE_SERVICE_TOKEN_SECRET_FILE`.
