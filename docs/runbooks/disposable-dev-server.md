...
- Provision every required Compose value with distinct disposable-development secrets, validate the rendered Compose configuration, disable parallel Bake and limit Compose to one concurrent engine/build operation by default, and write `DEPLOYED_GIT_SHA` only after health succeeds.
