...
- Tenant admins can inspect their account lifecycle status through `GET /api/v2/admin/account/status`. When deletion is requested, the response includes retained-record categories and purge eligibility dates.
- Tenant admins can stop subscription renewal through `POST /api/v2/admin/account/cancel` after confirming the tenant slug. Cancellation schedules the verified Stripe subscription to end at period close, keeps workspace access active through the paid period, and writes an audit-log event without starting the deletion clock or storing secret values.
- Tenant admins can request deletion for their workspace through `DELETE /api/v2/admin/account` after confirming the tenant slug. The request marks the tenant `PURGED`, revokes tenant sessions, and records the retained-record categories and eligibility dates that remain subject to billing, audit, log, and backup retention.
- Platform admins can archive tenants through `POST /api/v2/admin/tenants/:id/archive`, restore tenants through `POST /api/v2/admin/tenants/:id/restore`, and hard-delete deletion-requested tenants through `DELETE /api/v2/admin/tenants/:id` only after retained database records expire.
